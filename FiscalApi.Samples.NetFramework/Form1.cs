...
namespace FiscalApi.Samples.NetFramework
{
    public partial class Form1 : Form
    {
        ...
        private async void ObtenerCatalogRecordPorId_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("OK");
                MessageBox.Show($@"Catalogo: {apiResponse.Data.Description}");
            }
            else
            {
                MessageBox.Show($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
        ...
        private async void FirmarButton_Click(object sender, EventArgs e)
        {
            var fiscalapi = FiscalApiClient.Create(Settings);

            // Fiel (EKU9001283C9)
            var requestModel = new SignManifestRequest
            {
